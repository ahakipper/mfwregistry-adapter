...
func parseAtlasRealConfig() (atlasRealConfig, error) {
	...
	if c.auth != "" && !c.tls && (os.Getenv("ATLAS_REAL_ALLOW_INSECURE_AUTH") != "1" || os.Getenv("ATLAS_REAL_SCRATCH") != "1" || os.Getenv("ATLAS_REAL_ALLOW_WRITE") != "1") {
		return c, errors.New("ATLAS_REAL_AUTH_TOKEN requires TLS or scratch+write+explicit insecure-auth guards")
	}
...
func TestParseAtlasRealConfigAuthAndTLSGuards(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("ATLAS_REAL_ADDR", tc.addr)
			t.Setenv("ATLAS_REAL_AUTH_TOKEN", "tok")
			if tc.tls {
			...
			if tc.allow {
				...
				t.Setenv("ATLAS_REAL_ALLOW_INSECURE_AUTH", "")
			}
			if tc.scratch {
				t.Setenv("ATLAS_REAL_SCRATCH", "1")
			} else {
				t.Setenv("ATLAS_REAL_SCRATCH", "")
			}
			if tc.write {
				t.Setenv("ATLAS_REAL_ALLOW_WRITE", "1")
			} else {
				t.Setenv("ATLAS_REAL_ALLOW_WRITE", "")
			}
			_, err := parseAtlasRealConfig()
