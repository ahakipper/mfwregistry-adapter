...
> **Historical scope note (2026-09-19):** The Atlas reconcile analysis below is
> retained as provenance for why the Nacos Sink became authoritative. Atlas is
> excluded from the current release; do not schedule Atlas implementation,
> wire validation, or retirement work from this document.
...
> Current scope addendum: Atlas is compatibility scaffolding only and is
> explicitly excluded from the current release; historical Atlas findings below
> are preserved. The current release evaluates the Nacos Sink.

**Auditor:** DS-3 (reconcile correctness vs nacos)

> Historical boundary: Atlas-as-diff-source findings below apply to the
> historical/default Atlas reconcile mode. Atlas is excluded in the current
...
> **Current-status addendum (2026-09-19):** Current reconcile scope evaluates the Nacos single-Sink SDK path. Atlas details below are historical findings only and Atlas is excluded from the current release. The fresh Nacos 3 + KWOK 20-minute data-plane gate passed. Deployment-level Nacos HA/TLS/auth/namespace/leaderless checks are outside current Spotter scope.
