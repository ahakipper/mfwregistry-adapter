...
> **Current-status addendum (2026-09-13, authoritative implementation baseline `c986632`):** Current reconcile scope evaluates the Nacos single-Sink SDK path. Atlas remains an optional existing Sink/mock deferred for a future entry gate; Atlas details below are historical findings. The pinned SDK still lacks cluster-admin health-check update, so startup fails closed before readiness/canary writes. Deployment-level Nacos HA/TLS/auth/namespace/leaderless checks are outside current Spotter scope.
