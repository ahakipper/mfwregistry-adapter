...
> **Current-status addendum (2026-09-14):** This document's historical soak stack used Nacos 2.1.0 and remains provenance only. The active release gate uses `nacos/nacos-server:v3.2.4-slim` on `linux/arm64`, the official Nacos Go SDK gRPC facade, and the real `kwokctl` lifecycle. The 2h/1000+ run remains **NOT VERIFIED** until its JSONL evidence records every source/cache/Nacos comparison tick, bounded divergence age, batch sizes, retries, and cleanup result. Historical Nacos 2.1 findings must not be read as Nacos 3 PASS.
