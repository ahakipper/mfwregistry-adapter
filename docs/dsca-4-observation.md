...
> **Current-status addendum (2026-09-13, implementation HEAD `0334237`):** The live-source snapshot, divergence ledger/age, per-tick verdict, command bounds, and kwok-oriented stack design described in §4 are implemented and unit/race tested. Observe lifecycle and fake-PATH capacity/cleanup gates are implemented; the required 2h/1000+ Observe run has not been executed, so DS-4-1 scale/duration evidence and DS-4-5 capacity evidence remain **NOT VERIFIED**. Historical soak findings below are retained as provenance and must not be read as current production PASS; clean-host skips remain explicit `NOT VERIFIED`.
