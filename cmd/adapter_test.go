...
// resolved Config consumed by the composition root. These tests cover the
// struct path; every registered adapter flag reaches adapterFlags.
package cmd
...
func TestAdapterDoesNotExposeAppCenterTransportFlags(t *testing.T) {
	for _, name := range []string{
		"appcenter-notice-endpoint",
		"appcenter-notice-auth-token",
		"appcenter-notice-timeout",
		"appcenter-notice-retries",
	} {
		if flag := adapterCmd.Flags().Lookup(name); flag != nil {
			t.Fatalf("excluded AppCenter transport flag %q was reintroduced", name)
		}
	}
}
...
// TestFlagMappingResolvesConfig exercises the full command flag path from
// cobra values through infraconfig.Load. The resolved value object is the
// only configuration contract consumed by the composition root.
func TestFlagMappingResolvesConfig(t *testing.T) {
	cmd := newAdapterCommand()
	setFlag(t, cmd, "nacos-addr", "127.0.0.1:18848")
	setFlag(t, cmd, "consul-addr", "127.0.0.1:18500")
	setFlag(t, cmd, "kubeconfig", "/tmp/soak/kubeconfig")
